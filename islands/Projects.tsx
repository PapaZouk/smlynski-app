      ...
      <div className="mb-52 sm:mb-40">
              ...
              <div className="absolute inset-0 -z-10 w-full h-full">
                {slideImages.map((src, index) => (
                    <img
                        ...
                        src={src}
                        alt="Hero Slide"
                        className={`absolute inset-0 w-full h-full object-cover object-center transition-opacity duration-10000 ease-in-out ${currentSlide.value === index ? 'opacity-100' : 'opacity-0'}`}
                    />
                ))}
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-12 relative -top-16">
            <div className="mx-auto max-w-2xl lg:mx-0 bg-black bg-opacity-50 p-6 rounded-lg mt-10">
              ...
              <p className="mt-6 text-base leading-relaxed text-gray-300 sm:text-lg md:text-xl">
                Nasza firma specjalizuje się w realizacji projektów budowlanych na terenie całego kraju.
                Realizujemy budowę zarówno budynków mieszkalnych, jak i przemysłowych, zapewniając kompleksową
                obsługę od
                projektu po przekazanie kluczy. Dołącz do grona naszych zadowolonych klientów i zrealizuj swoje
                marzenia o idealnym budynku z naszą pomocą!
              </p>
            </div>
          </div>
        </div>
        <div className="grid grid-cols-1 px-4 sm:px-40 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {currentProjects.map((project: Project, index: number) => (
              <div key={index} className="transform transition-transform duration-500 hover:scale-105">
